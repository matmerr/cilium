...
func (r *infraIPAllocator) getDelegatedIPAMPluginConfig() (*libcni.PluginConfig, error) {
	...
	if netConf.PluginConfig.Network == nil {
		return nil, fmt.Errorf("CNI plugin config has no parsed network section, cannot determine IPAM type for delegated IPAM")
	}
	if len(netConf.PluginConfig.Bytes) == 0 {
		return nil, fmt.Errorf("CNI plugin config has no preserved raw bytes, cannot invoke delegated IPAM without plugin-specific fields")
	}
