...
func TestGetDelegatedIPAMPluginConfig(t *testing.T) {
	tests := []struct {
		name      string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			r := &infraIPAllocator{
				cniConfigManager: &fake.FakeCNIConfigManager{CustomNetConf: tc.netConf},
			}
			pc, err := r.getDelegatedIPAMPluginConfig()
			if tc.expectOK {
...
func newTestAllocator(t *testing.T, netConf *cnitypes.NetConf, cniBinDir string) *infraIPAllocator {
	t.Helper()
	return newTestAllocatorWithFamilies(t, netConf, cniBinDir, true, true)
}

func newTestAllocatorWithFamilies(t *testing.T, netConf *cnitypes.NetConf, cniBinDir string, enableIPv4, enableIPv6 bool) *infraIPAllocator {
	t.Helper()
